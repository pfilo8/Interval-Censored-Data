...
For bigger of $\nu$ frequency of checking lightbulb is smaller which can results in longer time between lightbulb fail and lightbulb replacemet than for more freqent checking. We expect that the bigger $\nu$, the bigger difference bewteen estimated and the real value will be. 

At figure \ref{fig:mean_naive} we observe the estimated values of the failure rate and the real values of $\lambda$ (red, dotted line). As we predicted the estimated values are higher than the real ones and the bigger value of $\nu$, the bigger difference bewteen estimated and the real value is. 
...
Based on previous observations we expect that with $\nu$ growth the bias of naive estimator will also grow. 

At figure \ref{fig:bias_naive} we observe that the bias of naive estimator is bigger for bigger $\nu$ values as we predicted. For one chosen $\nu$ bias is constant (do not depends on $\lambda$).

This estimator is not unbaised. Unbaised estimator is when bias is equal 0 (red, dotted line).
...
\section{Taking into account interval censoring}

We created our own implementation of Turnbull estimator of survival function based on notes from lecture. Our implementation of this procedure is presented on Listing \ref{lst:turnbull}.

\lstinputlisting[language=R, caption=Implementation of generator., label={lst:turnbull}, captionpos=b]{simulations/turnbull.R}
...
```{r echo=FALSE}
org_interval<-read.csv("results/org_interval.csv")
turnbull_interval<-read.csv("results/turnbull_interval.csv")

knitr::kable(list(org_interval, turnbull_interval),
  caption = "Generated intervals (first table) and intervals after Turnbull procedure (second table) - example", booktabs = TRUE) %>%
...
At figure \ref{fig:mean_turnbull} we observe the estimated values of the failure rate and the real values of $\lambda$ (red, dotted line). As we predicted the estimated values are smaller than for the naive estimator (figure \ref{fig:mean_naive}), and the bigger value of $\nu$, the bigger difference bewteen estimated and the real value is. 
...
means  %>%
  ...
  theme_minimal()
```
\caption{Mean of Turnbull estimator of the failure rate.}
\label{fig:mean_turnbull}
\end{figure}

\subsection{Variance of the estimator}

As we predicted the variance of estimated values (figure \ref{fig:var_turnbull}) are much smaller than for the naive estimator (figure \ref{fig:var_naive}), and as previously the variance of naive estimator is bigger for bigger $\nu$ and $\lambda$ values. The curve is not as smooth as for the naive estimator.
...
vars  %>%
  ...
  theme_minimal()
```
\caption{Variance of Turnbull estimator of the failure rate.}
\label{fig:var_turnbull}
\end{figure}

\subsection{Bias of the estimator}

As we predicted the bias values of Turnbull estimator (figure \ref{fig:bias_turnbull}) are smaller than for the naive estimator (figure \ref{fig:bias_naive}). This time the bias depends not only on $\nu$ (as it was with naive estimator) but changes also with change of $\lambda$. For one chosen $\nu$ bias is not constant.

This estimator is also not unbaised. Unbaised estimator is when bias is equal 0 (red, dotted line).
...
bias %>%
  ...
  theme_minimal()
```
\caption{Bias of Turnbull estimator of the failure rate.}
\label{fig:bias_turnbull}
\end{figure}

\subsection{Mean square error of the estimator}

At figure \ref{fig:mean_sq_turnbull} we observe that the mean square error of Turnbull estimator is bigger for bigger $\nu$ and $\lambda$ values. It is the exponential growth. The values of mean square error of the estimator are much smaller than for the naive estimator \ref{fig:mean_sq_naive}.
...
```{r echo=FALSE, fig.width=8, fig.height=4}
 mean_sq  %>%
  ...
  theme_minimal()
```
\caption{Mean square error of Turnbull estimator of the failure rate.}
\label{fig:mean_sq_turnbull}
\end{figure}
