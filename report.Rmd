...
At figure \ref{fig:mean_naive} we observe the estimated values of the failure rate and the real values of $\lambda$ (dotted line). As we predicted the estimated values are higher.
...
p1 <- data.frame(lambda_vector ,means[ ,1])  %>%
  ggplot(aes(x = lambda_vector, y = means[ ,1])) +
  geom_point() +
  geom_line() +
  geom_line(aes(y = lambda_vector), color = "blue", linetype = "dotted") +
  xlab(TeX('$\\lambda$ - lightbulb failure rate')) +
  ylab(TeX('Mean of naive estimator of $\\lambda$')) +
  ggtitle('Changing $\\lambda$, , $\\nu = 1$') +
  theme_minimal()

p2 <- data.frame(nu_vector ,means[1, ])  %>%
  ggplot(aes(x = lambda_vector, y = means[1, ])) +
  geom_point() +
  geom_line() +
  geom_line(aes(y = lambda_vector), color = "blue", linetype = "dotted") +
...
At figure \ref{fig:var_naive} we observe that the bias of naive estimator is bigger for bigger $\nu$ and $\lambda$ values as we predicted. The growth is exponential. 

\begin{figure}
```{r echo=FALSE, fig.width=8, fig.height=4}
p1 <- data.frame(lambda_vector ,vars[ ,1])  %>%
...
p2 <- data.frame(nu_vector ,vars[1, ])  %>%
  ggplot(aes(x = lambda_vector, y = vars[1, ])) +
  geom_point() +
  geom_line() +
  xlab(TeX('$\\nu$ - inspection rate')) +
...
p2 <- data.frame(nu_vector ,bias[1, ])  %>%
  ggplot(aes(x = lambda_vector, y = bias[1, ])) +
  geom_point() +
  geom_line() +
  xlab(TeX('$\\nu$ - inspection rate')) +
...
p2 <- data.frame(nu_vector ,mean_sq[1, ])  %>%
  ggplot(aes(x = lambda_vector, y = mean_sq[1, ])) +
  geom_point() +
  geom_line() +
  xlab(TeX('$\\nu$ - inspection rate')) +
